...
// A backup id is generated by this server and looks like
// 2026-08-19T04:00:00.000Z-a1b2c3d4e5f6. Anything else is refused, because the
// id was being pasted straight into a path: an adversarial review passed
// '../../../../../tmp/secret' and read a file from outside every allowed root.
const VALID_BACKUP_ID = /^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9:.]+Z-[0-9a-f]{12}$/;

export function readBackup(abs, id) {
  if (typeof id !== 'string' || !VALID_BACKUP_ID.test(id)) {
    throw new Error(`Not a valid backup id: ${JSON.stringify(String(id).slice(0, 80))}. Ids come from safe_list_backups and look like 2026-08-19T04:00:00.000Z-a1b2c3d4e5f6.`);
  }
  const dir = path.join(BACKUP_ROOT, sha256(abs).slice(0, 16));
  const file = path.resolve(dir, `${id}.bak`);
  if (path.dirname(file) !== path.resolve(dir)) {
    throw new Error('Backup id escaped its directory. Refused.');
  }
